...
import { Processing, Shipped, Delivered, Cancelled } from "@/machines/order";
...
const createSimulations = (currentState: string): readonly Simulation[] => [
  {
    ...
    state: Processing.make({ startedAt: Date.now() }),
  ...
  },
  {
    ...
    state: Shipped.make({ trackingNumber: "SIM-" + Date.now(), shippedAt: Date.now() }),
  ...
  },
  {
    ...
    state: Delivered.make({ deliveredAt: Date.now() }),
  ...
  },
  {
    ...
    state: Cancelled.make({ reason: "Simulated server cancellation", cancelledAt: Date.now() }),
...
export function ConflictSimulator({ orderId, currentState, className }: ConflictSimulatorProps) {
  ...
  const simulateServerChange = useCallback(
    (targetState: OrderState, actionName: string) => {
