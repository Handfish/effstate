...
export function OrderCard({ order }: OrderCardProps) {
  ...
  const handleProceedToCheckout = useCallback(() => {
    send(ProceedToCheckout.make());
  }, [send]);

  const handleBackToCart = useCallback(() => {
    send(BackToCart.make());
  }, [send]);

  const handlePlaceOrder = useCallback(() => {
    send(PlaceOrder.make());
  }, [send]);

  const handleMarkShipped = useCallback(() => {
    if (trackingInput.trim()) {
      send(MarkShipped.make({ trackingNumber: trackingInput.trim() }));
      setTrackingInput("");
  ...
  const handleMarkDelivered = useCallback(() => {
    send(MarkDelivered.make());
  }, [send]);

  const handleCancel = useCallback(() => {
    send(CancelOrder.make({ reason: "Customer requested cancellation" }));
  }, [send]);
    ...
    <div className="bg-gray-800 rounded-lg p-4 space-y-4">
      ...
      <div className="text-xs text-gray-500 pt-2 border-t border-gray-700">
        ...
        {state._tag === "Shipped" && (
          <>
            {" "}
            | Shipped: {new Date(state.shippedAt).toLocaleString()}
          </>
        )}
        {state._tag === "Delivered" && (
          <>
            {" "}
            | Delivered: {new Date(state.deliveredAt).toLocaleString()}
          </>
        )}
        {state._tag === "Cancelled" && (
          <>
            {" "}
            | Cancelled: {new Date(state.cancelledAt).toLocaleString()}
          </>
        )}
      </div>
    </div>
