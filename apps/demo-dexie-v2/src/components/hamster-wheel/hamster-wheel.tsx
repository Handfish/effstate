...
const HamsterWheelContent = ({
  ...
  // Use stateTag for comparisons (v2 API)
  const isRunning = status.stateTag === "Running";
  const isStopping = status.stateTag === "Stopping";
  const isIdle = status.stateTag === "Idle";
    ...
    >
      ...
      <div className="relative lg:absolute bottom-0 lg:bottom-4 left-0 right-0 text-center pb-4 lg:pb-0">
        <p className="text-gray-400 text-xs md:text-sm px-4">
          Using v2 API with Dexie (IndexedDB) for persistence. Cross-tab sync via liveQuery!
        </p>
      </div>
    </div>
  );
