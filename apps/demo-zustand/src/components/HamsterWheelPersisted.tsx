...
import { useHamsterWithPersistence, getHamsterStateLabel, getHamsterButtonLabel } from "@/stores/hamster-persisted";
import { getIsLeader } from "@/lib/db";

export function HamsterWheelPersisted() {
  const { state, wheelRotation, electricityLevel, toggle, _isInitialized } =
    useHamsterWithPersistence();

  const isLeader = getIsLeader();

  if (!_isInitialized) {
    return (
      <div className="flex flex-col items-center gap-4 p-6 rounded-lg bg-gray-900">
        <div className="text-gray-400">Loading from Dexie...</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-6 rounded-lg bg-amber-950/50">
      <h2 className="text-xl font-bold">Hamster Wheel (Zustand + Dexie)</h2>
