/**
 * Hamster Wheel Machine - effstate v4 (schema-first)
 *
 * States and events are schema-first discriminated unions defined with
 * State()/Event(). The wheel animation is a `run` stream that auto-cancels
 * when the machine leaves the Running state.
 */
